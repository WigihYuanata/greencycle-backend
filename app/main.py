from fastapi import FastAPI, Depends, HTTPException, BackgroundTasks, Security, Request, Query, UploadFile, File
...
"""@app.post("/machine/scan-qr-image")
...
# Skema khusus untuk menerima data murni dari ESP32 saat demo
...
# ==========================================================
# ENDPOINT KHUSUS DEMO PAMERAN (TANPA QR CODE)
# ==========================================================
@app.post("/machine/prototype/deposit", tags=["Machine Prototype"])
def machine_prototype_deposit(
    data: DemoDeposit,
    db: Session = Depends(get_db),
    authenticated: bool = Depends(machine_validate) 
):
    NPM_AKUN_DEMO = "25082010106" 
    
    # Cari user di database
    user = db.query(User).filter(User.npm == NPM_AKUN_DEMO).first()
    if not user:
        raise HTTPException(
            status_code=404, 
            detail="Akun demo pameran belum terdaftar."
        )
    
    # 2. Kalkulasi Poin
    poin_kecil = data.bottle_small * settings.HARGA_BOTOL_KECIL
    poin_sedang = data.bottle_medium * settings.HARGA_BOTOL_SEDANG
    poin_besar = data.bottle_large * settings.HARGA_BOTOL_BESAR
    total_poin = poin_kecil + poin_sedang + poin_besar
    ...
    return {
        "status": "Success",
        "message": f"Data berhasil masuk ke akun {user.name}!",
